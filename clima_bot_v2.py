...
def criar_grafico(dados):
    ...
    axs[0].set_ylabel('Chuva (%)')
    axs[0].grid(True, linestyle='--', alpha=0.6); axs[0].set_ylim(0, 100)
    axs[1].bar(df['Data'], df['Precipitação (mm)'], color='skyblue')
    axs[1].set_ylabel('Precipitação (mm)'); axs[1].grid(True, linestyle='--', alpha=0.6)
    axs[2].plot(df['Data'], df['Vento (km/h)'], marker='s', linestyle='-', color='green')
    axs[2].set_ylabel('Vento (km/h)'); axs[2].set_xlabel('Data'); axs[2].grid(True, linestyle='--', alpha=0.6)
    fig.autofmt_xdate(); axs[2].xaxis.set_major_formatter(mdates.DateFormatter('%d/%m/%Y'))
